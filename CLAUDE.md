...
- Aviso de actualización dentro de la app: consultar la última versión publicada y mostrar un botón de descarga (sin autoinstalación por ahora).
  Implementado con `GET https://godebol.com/api/evemtv/version` (sin datos de la cuenta; solo en release); solo abre descargas de `github.com/Elmarcinho` o `godebol.com`. Si la instalada es menor que `minima`: 3 días de plazo con aviso cerrable ("Debes actualizar antes del…", fecha guardada por versión mínima) y después bloqueo con descarga, instrucciones y enlace a la guía; nunca bloquea si no se pudo consultar. Ver `docs/decisiones.md` §19 y la guía para usuarios `docs/instalacion.md`.
